...
スコアと本文（セクション1・2）は必ず整合させてください：
- 6以下を付けた項目は「## 2. 改善すべき点」で必ず言及する（その項目のどの動作が減点要因かが本文から分かるように）
- 7以上の項目のうち代表的なもの（最高スコアの項目を含む）は「## 1. 全体評価（良い点）」で触れる
- 5項目それぞれについて、スコアの根拠が本文のどこかから一言以上読み取れるようにする（本文に根拠のないスコアを付けない）
