...
| 距離 | 主エネルギー系 | フェーズ構成 | 最小週数 | レース日デフォルト |
|------|--------------|------------|--------|--------------|
| 100m | ATP-PC系（無酸素・神経系） | GPP → SPP → 競技期 | 8週 | 12週後 |
| 200m | ATP-PC系 ＋ グリコーゲン分解系 | GPP → SPP → 競技期 | 8週 | 12週後 |
| 400m | グリコーゲン分解系（無酸素系主体） | GPP → SPP → 競技期 | 10週 | 16週後 |
| 800m | 有酸素系 ＋ 無酸素系（混合） | 有酸素基盤 → スピード持久力 → 競技期 | 10週 | 16週後 |
| 1500m | 有酸素系主体 ＋ 無酸素系補完 | 有酸素基盤 → スピード持久力 → 競技期 | 12週 | 16週後 |

- 「最小週数」：計画期間の下限。レースまでがこれより短い場合は開始日を過去に遡らせて確保する。
- 「レース日デフォルト」：STEP 1のレース日入力欄の初期値（`default_weeks`）。計画週数の上限ではない（v1.10.0で上限制は廃止）。
...
- `weekly_schedules[].days[].date`：AIは曜日名（月〜日）のみを返し、システム側が計画開始日（`start_dt`）から実際の日付を計算して「5/19(月)」形式で表示する。AIが完全日付を返した場合も曜日だけを抽出して再計算するため、日付の正は常にコード側（v1.10.0で固定）
...
- 計画開始日（`calculate_plan_weeks`）：原則として今週の月曜日（作成週）から開始。レースまでが最小週数より短い場合のみ、最小週数を確保するため過去の月曜日に遡る。計画週数は開始週〜レース週の実週数（上限なし）
- 出力トークン上限：計画週数に比例（`get_planner_max_tokens` = 週数×1500+4096、上限65536）。週数増加によるJSON途中切れを防止
